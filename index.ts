...
async function wantAnotherTransaction (fun: Function) {
    let condition  = true
    do {
        await fun()
        let wantrepeat = await inquirer.prompt (
            {name: "ask", type: "confirm", message: "Do you want to perform another transaction?"}
        )
